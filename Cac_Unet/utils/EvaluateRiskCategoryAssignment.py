...
parser.add_argument('--inputdir', default='E:\\bigorgs\\cardiovascular\\workspace\\Unet\\Cac_Unet\\')
parser.add_argument('--scratchdir', default='E:\\bigorgs\\cardiovascular\\workspace\\Unet\\Cac_Unet\\')
...
automatic_scores = read_cac_scores(path.join(config['inputdir'], 'calcium_scores.csv'))
...
# for k, v in automatic_scores.items():
#     automatic_scores = dict((k, v))
#
# for k, v in manual_scores.items():
#     manual_scores = dict((k, v))

# automatic_scores = dict((k, v) for k, v in automatic_scores.items() if k in test_data.uids)
automatic_scores = dict((k, v) for k, v in automatic_scores.items() if True)
# manual_scores = dict((k, v) for k, v in manual_scores.items() if k in test_data.uids)
manual_scores = dict((k, v) for k, v in manual_scores.items() if True)
