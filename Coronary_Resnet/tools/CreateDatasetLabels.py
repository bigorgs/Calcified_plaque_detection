...
def CreateEvalData():
    data_list = []
    test_root = r"E:\bigorgs\cardiovascular\workspace\Resnet\Coronary_Resnet\data\coronary\people\axis\test"

    class_flag = -1

    for a, b, c in os.walk(test_root):
        for i in range(len(c)):
            testdata=os.path.join(a, c[i]) + '\t' + str(class_flag) + '\n'
            data_list.append(testdata)

        class_flag += 1

    print(data_list)
    with open('testlabel.txt', 'w', encoding='UTF-8') as f:
